...
# --> sums the data for each column (0 or "index")
treats_sold = treats.apply(lambda df: df.sum())
...
# --> sums the data for each row (1 or "columns")
treats_sold_by_row = treats.apply(sum_rows, axis=1)
...
# columns, so do something to each row
example_reviews = example_reviews.apply(remean_points, axis=1)
...
# gets the row label of the highest value
...
first_wines = reviews.groupby('winery').apply(
...
best_wines_by_country_prov = reviews.groupby(['country', 'province']).apply(
    lambda df: df.loc[df.points.idxmax()], include_groups=False)
...
countries_reviewed = countries_reviewed.sort_values(
    by='len')  # sort results asc
countries_reviewed = countries_reviewed.sort_values(
    by='len', ascending=False)  # sort results desc
countries_reviewed.sort_index()  # sort by index
# sort by more than 1 col at a time
...
).sort_values(ascending=False)  # sort results desc
...
# transform whole df
memory_effecient_reviews = reviews.astype(
    {'points': 'int32', 'stars': 'int32', 'price': 'float32'})


# Missing data
pd.isnull(reviews.country)  # --> Series: indexes where reviews.country is null
# --> gets new df with rows that have missing country
...
# replace values
reviews.taster_twitter_handle = reviews.taster_twitter_handle.replace(
    "@kerinokeefe", "@kerino")
...
# or this
reviews_per_region = reviews.fillna({'region_1': 'Unknown'}).groupby(
    'region_1')['region_1'].count().sort_values(ascending=False)
...
best_vals = reviews.loc[reviews.groupby(['country', 'province'])[
...
most_reviewed_winery_per_country = reviews.groupby(
    ['country', 'winery']).size().sort_values(ascending=False)
...
ais_highest_price_per_winery = reviews.loc[reviews.groupby(
    'winery')['price'].idxmax()].sort_values(by=['price'], ascending=False)

# 14. Group by 'country' 'variety', count the total number of reviews
total_reviews_per_country_variety = reviews.groupby(
    ['country', 'variety']).size().reset_index(name='the total')
...
# get list of taster_twitter_handles with more than 2 reviews
...
highest_avg_points_winery_per_country = (
    ...
    # Re-group by just the country (the mean calculation created a multi-index of (country, winery))
    .groupby(level=0)
